#!/usr/bin/env python3
"""
使用指定路径的Blender来展开骰子模型的UV
"""
import subprocess
import os

# Blender可执行文件路径
BLENDER_PATH = "D:/blender/blender-4.0.0-windows-x64/blender.exe"
...
try:
    ...
    result = subprocess.run(
        [BLENDER_PATH, '--background', '--python', 'unwrap_dice_temp.py'],
        capture_output=True,
        text=True
    )
    print("Blender输出:")
    ...
    print(f"错误: 找不到Blender可执行文件。请检查路径是否正确: {BLENDER_PATH}")
