...
func collect_rewards(result_id: String) -> Array:
	var result = adventure_results.get(result_id, {})
	if result.is_empty():
		print("【AdventureManager.collect_rewards】结果 ", result_id, " 不存在或为空")
		return []
	...
	# 先检查组合效果
	match effect:
		"gold_add_and_hp_minus":
			rewards.append({"id": "gold", "amount": p2_val})
			rewards.append({"id": "hp_minus", "name": "生命损失", "type": "损失", "amount": -p1_val})
		"str_add_and_hp_minus":
			rewards.append({"id": "str_add", "name": "力量增加", "type": "属性", "amount": p2_val})
			rewards.append({"id": "hp_minus", "name": "生命损失", "type": "损失", "amount": -p1_val})
		_:
			# 处理单一效果（用 _and_ 拆分的组合效果，如 gold_add_and_hp_add）
			var effects = effect.split("_and_")
			for single_effect in effects:
				match single_effect:
					"gold_add":
					...
					"gold_minus":
						rewards.append({"id": "gold_minus", "name": "金币损失", "type": "损失", "amount": -p1_val})
					"hp_add":
					...
					"str_minus":
						rewards.append({"id": "str_minus", "name": "力量损失", "type": "损失", "amount": -p1_val})
					"agi_add":
						rewards.append({"id": "agi_add", "name": "敏捷增加", "type": "属性", "amount": p1_val})
					"agi_minus":
						rewards.append({"id": "agi_minus", "name": "敏捷损失", "type": "损失", "amount": -p1_val})
					"int_add":
						rewards.append({"id": "int_add", "name": "智力增加", "type": "属性", "amount": p1_val})
					"int_minus":
						rewards.append({"id": "int_minus", "name": "智力损失", "type": "损失", "amount": -p1_val})
					"item_add":
						rewards.append({"id": str(p1_val), "amount": 1})
					"item_buy":
						rewards.append({"id": "gold_minus", "name": "金币损失", "type": "损失", "amount": -p1_val})
						rewards.append({"id": str(p2_val), "amount": 1})
					"all_add":
						rewards.append({"id": "str_add", "name": "全属性增加", "type": "属性", "amount": p1_val})
					"all_minus":
						rewards.append({"id": "str_minus", "name": "全属性损失", "type": "损失", "amount": -p1_val})
					"all_attr_add":
						rewards.append({"id": "str_add", "name": "全属性增加", "type": "属性", "amount": p1_val})
					"gold_and_attr_add":
						rewards.append({"id": "gold", "amount": p1_val})
