...
func generate_core_chain(target_total: int, difficulty: int = 1, seed_value: int = 0) -> Array[LevelNode]:
	...
	for stage in range(stage_count):
		...
		for layer in range(stage_layer_count):
			...
			# 先生成整层节点
			var layer_nodes: Array[LevelNode] = []
			for i in range(nodes_in_layer):
				var node = _create_layer_node(current_node_id, stage + 1, global_layer)
				layer_nodes.append(node)
				current_node_id += 1

			# 检查并修复：确保同一层节点类型不完全相同
			_ensure_layer_type_diversity(layer_nodes)

			# 添加到阶段节点
			stage_nodes.append_array(layer_nodes)
...
func _create_layer_node(node_id: int, stage: int, layer: int) -> LevelNode:
	...
	node.type = _get_node_type_for_node_id(node_id)  # 根据节点 ID 计算节点类型（每个节点独立随机）
	node.is_core = true
	node.is_start = false
	node.is_end = false
	node.layer = layer
...
## 根据节点 ID 计算节点类型（带随机种子）
## 返回：1=战斗，2=奇遇，3=交易，4=奖励
func _get_node_type_for_node_id(node_id: int) -> int:
	# 使用节点 ID 作为随机种子，保证每个节点都有独立的随机类型
...
func _ensure_layer_type_diversity(layer_nodes: Array[LevelNode]):
	if layer_nodes.size() < 2:
		return  # 只有一个节点，不需要检查

	# 检查是否所有节点类型都相同
	var first_type = layer_nodes[0].type
	var all_same = true
	for node in layer_nodes:
		if node.type != first_type:
			all_same = false
			break

	if not all_same:
		return  # 已经有不同类型，不需要处理

	# 所有节点类型相同，随机重新分配其中一个节点的类型
	# 使用当前节点 ID 作为种子，保证可重复性
	var rand_node_idx = randi() % layer_nodes.size()
	var new_type = _get_diverse_type(layer_nodes[rand_node_idx].type, layer_nodes)
	layer_nodes[rand_node_idx].type = new_type
	print("  [层多样性修复] 节点#", layer_nodes[rand_node_idx].id, " 从类型 ", first_type, " 改为 ", new_type)


## 获取一个与当前类型不同的新类型（用于多样性修复）
func _get_diverse_type(current_type: int, layer_nodes: Array[LevelNode]) -> int:
	var available_types = [1, 2, 3, 4]
	available_types.erase(current_type)

	# 如果有多种类型可选，随机选择一个
	if available_types.size() > 1:
		var idx = randi() % available_types.size()
