...
func insert_random_nodes(
	...
	# 增强随机性：计算每个间隙应该插入的节点数时使用随机分布
	var gaps = core_chain.size() - 1  # 核心节点之间的间隙数
	if gaps <= 0 or random_count_needed <= 0:
		# 没有间隙或不需要插入，直接返回
		return core_chain.duplicate()

	# 计算每个间隙的节点分配（使用随机分布而非平均分配）
	var nodes_per_gap: Array[int] = []
	var remaining_nodes = random_count_needed

	for i in range(gaps):
		if i == gaps - 1:
			# 最后一个间隙：分配所有剩余节点
			nodes_per_gap.append(remaining_nodes)
		else:
			# 前面的间隙：随机分配 1 到平均值之间的数量
			var avg_remaining = float(remaining_nodes) / float(gaps - i)
			var min_for_gap = 1
			var max_for_gap = maxi(2, int(avg_remaining * 1.5))  # 最多为平均值的 1.5 倍

			var count = randi_range(min_for_gap, mini(max_for_gap, remaining_nodes - (gaps - i - 1)))
			count = maxi(1, count)  # 至少 1 个

			nodes_per_gap.append(count)
	...
	for i in range(core_chain.size()):
		...
		if i < core_chain.size() - 1:
			var count_to_insert = nodes_per_gap[i]

			for j in range(count_to_insert):
