extends Node
## 奖励管理器 (Autoload 单例)
## 负责加载奖励配置、管理奖励流程、执行效果
## 参照 adventure_manager.gd 架构，仅效果全为正面

# 数据缓存
var reward_events: Dictionary = {}  # id -> event data
var reward_results: Dictionary = {}  # id -> result data
...
func _ready():
	print("【奖励管理器】奖励管理器初始化")
	load_reward_data()


## 获取单例
static func get_instance() -> Node:
	return Engine.get_main_loop().root.get_node_or_null("RewardManager")


## 加载奖励数据
func load_reward_data():
	_load_reward_events()
	_load_reward_results()
	print("【奖励管理器】加载完成，事件数: ", reward_events.size(), ", 结果数: ", reward_results.size())


func _load_reward_events():
	var json_path = "res://table/reward_events.json"
	...
	if file:
		...
		if parse_result != OK:
			push_error("【奖励管理器】解析 reward_events.json 失败: ", json.get_error_message())
			return

		var data = json.get_data()
		if not data is Dictionary or not data.has("reward_events"):
			push_error("【奖励管理器】reward_events.json 缺少 'reward_events' 数组")
			return

		var events_array = data["reward_events"]
		for event_data in events_array:
			if event_data is Dictionary and event_data.has("id"):
				var event_id = str(event_data["id"])
				reward_events[event_id] = event_data

		print("【奖励管理器】JSON 加载完成，事件数: ", reward_events.size())
	else:
		push_error("【奖励管理器】无法打开 reward_events.json")


func _load_reward_results():
	var json_path = "res://table/reward_results.json"
	var file = FileAccess.open(json_path, FileAccess.READ)
	...
	if file:
		...
		if parse_result != OK:
			push_error("【奖励管理器】解析 reward_results.json 失败: ", json.get_error_message())
			return

		var data = json.get_data()
		if not data is Dictionary or not data.has("reward_results"):
			push_error("【奖励管理器】reward_results.json 缺少 'reward_results' 数组")
			return

		var results_array = data["reward_results"]
		for result_data in results_array:
			if result_data is Dictionary and result_data.has("id"):
				var result_id = str(result_data["id"])
				reward_results[result_id] = result_data

		print("【奖励管理器】JSON 加载完成，结果数: ", reward_results.size())
	else:
		push_error("【奖励管理器】无法打开 reward_results.json")


## 获取奖励事件
func get_reward_event(event_id: String) -> Dictionary:
	return reward_events.get(event_id, {})


## 获取奖励结果
func get_reward_result(result_id: String) -> Dictionary:
	return reward_results.get(result_id, {})


## 获取奖励结果列表
func get_reward_results(event_id: String) -> Array:
	var event = reward_events.get(event_id, {})
	if event.is_empty():
		return []
	...
	for rid in result_ids:
		var result = reward_results.get(str(rid), {})
...
## 开始奖励
func start_reward(event_id: String) -> bool:
	var event = reward_events.get(event_id, {})
	if event.is_empty():
		push_error("【奖励管理器】找不到奖励事件: ", event_id)
		return false

	current_event = event
	current_result_ids = event.get("results", [])
	current_results = get_reward_results(event_id)

	print("【奖励管理器】开始奖励: ", event.get("name", ""), ", 选项数: ", current_results.size())
	return true
...
func get_reward_dice_config(result_count: int) -> Dictionary:
	var values = []
	var textures = []
	...
	for i in range(result_count):
		...
		for j in range(count):
			values.append(i + 1)
			textures.append("reward_" + str(i + 1))
			face_idx += 1

	# 确保是6面
	while values.size() < 6:
		values.append(result_count)
		textures.append("reward_" + str(result_count))
...
## 奖励关卡全部为正面效果
## 支持的效果类型：
##   - 单一效果：hp_add, gold_add, str_add, agi_add, int_add, all_add, all_attr_add, item_add
##   - 组合效果：用 _and_ 连接，如 hp_add_and_gold_add
func execute_effect(result_id: String) -> Dictionary:
	var result = reward_results.get(result_id, {})
	if result.is_empty():
		push_error("【奖励管理器】找不到结果: ", result_id)
	...
	print("【奖励管理器】执行效果: ", effect, ", 参数: ", params)
...
func _apply_single_effect(effect_type: String, params: Dictionary, des: String) -> Dictionary:
	...
	match effect_type:
		...
		"item_add":
			print("【奖励管理器】获得道具: ", p1_val, "（道具系统待实现）")
			effect_result["success"] = true
		...
		_:
			push_warning("【奖励管理器】未知效果类型: ", effect_type)
...
func _apply_gold_change(amount: int):
	...
	if char and "gold" in char:
		char.gold += amount
		print("【奖励管理器】金币变化: ", amount, ", 当前金币: ", char.gold)
...
func _apply_stat_add(attr_type: String, amount: int):
	...
	for char in player_characters:
		...
		print("【奖励管理器】", char.name, " ", attr_type, " 变化: ", amount)
