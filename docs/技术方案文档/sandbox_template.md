...
## 基本信息
- **创建日期**: 2026-03-03
- **修改日期**: 2026-03-21（更新摄像机配置和 CameraManager 统一管理）
- **场景文件**: `res://scenes/dice_demo_simple_final.tscn`
- **用途**: 作为日后创建新场景的标准模板

## 摄像机配置

### CameraManager 统一管理（重要）

**所有新场景必须使用 CameraManager 统一管理摄像机**，不再在场景脚本中单独配置。
...
func _ready():
    # 注册摄像机到 CameraManager（Autoload 单例）
    if camera:
...
- **位置**: `Vector3(1.3, 50, 14.1)`
  - x=1.3：轻微向右偏移
  - y=50：高度 50 单位（地面在 y=-4，摄像机距地面 54 单位）
  - z=14.1：向前接近南墙（南墙在 z=6.75）
...
- **FOV**: 15.0 度（窄视角，扁平化效果）

#### Godot 坐标系说明
- **旋转顺序**: XYZ（rotation.x, rotation.y, rotation.z）
- **屏幕映射**:
  - x 轴：水平方向（右为正）
  - z 轴：前后方向（前为正，对应屏幕上下）
  - y 轴：垂直方向（上为正）
...
CameraManager 提供 4 种预设配置：

| 预设名称 | 位置 | FOV | 用途 |
|---------|------|-----|------|
| `default` | (1.3, 50, 14.1) | 15.0° | 标准配置（推荐） |
| `high` | (1.3, 80, 14.1) | 15.0° | 更高视角 |
| `low` | (1.3, 40, 14.1) | 15.0° | 更低视角 |
| `wide` | (1.3, 50, 14.1) | 25.0° | 更宽视野 |

**使用预设**：
```gdscript
# 在场景脚本中切换摄像机预设
func _input(event):
    if event.is_action_pressed("ui_home"):  # Home 键
        CameraManager.set_preset("high")
    elif event.is_action_pressed("ui_end"):  # End 键
...
#### 旧配置（已废弃）
- **位置**: (0, 60, 0)
- **旋转**: Vector3(-PI/2, 0, 0) 完全俯视
- **说明**: 早期配置，已替换为当前 3D 倾斜视角

### 投影设置
- **投影模式**: 透视投影 (`projection = 0`)
- **FOV**: 15.0 度（窄视角，扁平化效果）
- **效果**: 提供 3D 立体感的同时保持接近平视的视觉效果
