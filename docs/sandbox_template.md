...
### 位置与旋转
- **Transform**: `Transform3D(1, 0, 0, 0, 0.7071, -0.7071, 0, 0.7071, 0.7071, 0, 30, 0)`
- **位置**: (0, 30, 0) （进一步拉远摄像机，确保能看到墙体）
- **旋转角度**: 约45度倾斜（保持适度的3D效果）
...
## 骰子设置
- **初始位置**: (0, 3, -sandbox_height/2 + 0.5) (水平方向屏幕中间，靠近下方墙体（南墙），悬浮状态)
- **投掷位置**: (0, 5, -sandbox_height/2 + 0.5) (水平方向屏幕中间，靠近下方墙体（南墙），离地面5个骰子高度)
- **类型**: 实例化的骰子场景
- **投掷力**: 
