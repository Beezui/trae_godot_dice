...
1. **配置骰子面**：
   - 在 `table/骰子面配置.csv` 文件中添加或修改骰子面配置

2. **配置场景**：
   - 在 `dice_config.gd` 文件中修改 `scene_configs` 字典，为不同场景配置不同的骰子面ID

3. **运行时切换**：
   - 按 1 键：切换到 normal 场景配置
   - 按 2 键：切换到 skill 场景配置

4. **添加新场景**：
   - 使用 `add_scene_config` 函数添加新的场景配置

## 技术实现要点

1. **基于ID的配置系统**：
   - 使用ID作为中间层，解耦配置和具体的贴图路径
   - 便于后续扩展和维护
...
动态更换贴图功能通过基于ID的配置系统、彩色材质替代方案和多表面网格技术，实现了骰子外观的动态切换。该功能支持通过配置文件和场景切换来更改骰子的外观，为游戏增加了更多的视觉多样性和扩展性。

**核心特性**：
- 基于ID的配置系统，解耦配置和具体贴图路径
- 动态创建具有6个独立表面的骰子模型，支持每个面独立更换贴图
- 从CSV文件加载骰子面数据，支持灵活配置
- 彩色材质作为贴图的替代方案，确保在不同环境下都能正常工作
- 支持在游戏运行时切换场景配置，实时更新骰子外观
- 跨平台兼容性，不依赖外部3D模型文件
