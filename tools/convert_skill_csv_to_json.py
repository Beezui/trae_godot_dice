...
def convert_hero_csv_to_json(csv_path, json_path):
    """将 hero.csv 转换为 JSON 格式（支持 type/stage 字段）"""
    ...
    for i in range(1, len(lines)):
        ...
        if len(values) < 15:
            print(f"  ✗ Warning: Insufficient columns (got {len(values)}, need 15), skipping")
            continue
        ...
        # 使用分号作为数组分隔符（注意索引已偏移 2，跳过 type 和 stage）
...
def convert_items_csv_to_json(csv_path, json_path):
    """将 items.csv 转换为 JSON 格式"""

    print("=" * 50)
    print("Items CSV to JSON Converter")
    print("=" * 50)
    print()
    ...
    for i in range(1, len(lines)):
        ...
        if len(values) < 7:
            print(f"  ✗ Warning: Insufficient columns (got {len(values)}, need 7), skipping")
            continue

        item_id = values[0].strip()
        item_name = values[1].strip()
        item_type = values[2].strip()
        price = int(values[3].strip()) if values[3].strip().isdigit() else 0
        description = values[4].strip()
        icon = values[5].strip()
        weight = int(values[6].strip()) if values[6].strip().isdigit() else 10

        if not item_id or not item_name:
            print(f"  ✗ Warning: Empty item ID or name, skipping")
            continue
    ...
    print()
    print("=" * 50)
    print("✓ Items conversion completed successfully!")
...
def convert_reward_events_csv_to_json(csv_path, json_path):
    """将 reward_events.csv 转换为 JSON 格式"""

    print("=" * 50)
    print("Reward Events CSV to JSON Converter")
    print("=" * 50)
    print()
    ...
    for i in range(1, len(lines)):
        ...
        print(f"  ✓ Added reward event: {event_id}")
    ...
    print()
    print("=" * 50)
    print("✓ Reward Events conversion completed successfully!")
    print(f"  Total reward events: {len(reward_events)}")
    print(f"  Output file: {json_path}")
    print("=" * 50)
...
def convert_reward_results_csv_to_json(csv_path, json_path):
    """将 reward_results.csv 转换为 JSON 格式"""

    print("=" * 50)
    print("Reward Results CSV to JSON Converter")
    print("=" * 50)
    print()
    ...
    for i in range(1, len(lines)):
        ...
        print(f"  ✓ Added reward result: {result_id}")
    ...
    print()
    print("=" * 50)
    print("✓ Reward Results conversion completed successfully!")
    print(f"  Total reward results: {len(reward_results)}")
    print(f"  Output file: {json_path}")
    print("=" * 50)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--num-dices':
        ...
        csv_file = os.path.join(script_dir, '..', 'table', 'items.csv')
        json_file = os.path.join(script_dir, '..', 'table', 'items.json')
        ...
        if not success:
            sys.exit(1)
    elif len(sys.argv) > 1 and sys.argv[1] == '--reward-events':
        csv_file = os.path.join(script_dir, '..', 'table', 'reward_events.csv')
        json_file = os.path.join(script_dir, '..', 'table', 'reward_events.json')
        ...
        success = convert_reward_events_csv_to_json(csv_file, json_file)

        if not success:
            sys.exit(1)
    elif len(sys.argv) > 1 and sys.argv[1] == '--reward-results':
        csv_file = os.path.join(script_dir, '..', 'table', 'reward_results.csv')
        json_file = os.path.join(script_dir, '..', 'table', 'reward_results.json')
